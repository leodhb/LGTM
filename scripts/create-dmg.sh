...
VERSION=$(grep -m 1 "MARKETING_VERSION" LGTM.xcodeproj/project.pbxproj | awk -F' = ' '{print $2}' | tr -d ';' | tr -d ' ')
...
APP_PATH=$(find ~/Library/Developer/Xcode/DerivedData -name "LGTM.app" -type d 2>/dev/null | \
    while read app; do
...
create-dmg \
  --volname "LGTM Installer" \
  --window-pos 200 120 \
  --window-size 660 400 \
  --icon-size 160 \
  --icon "LGTM.app" 180 170 \
  --hide-extension "LGTM.app" \
